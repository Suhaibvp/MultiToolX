package com.example.simple_world.ui.ble.advertise
...
import com.example.simple_world.ui.ble.ble_advertise.AdvancedSettingsDialogFragment
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE_PERMISSIONS) {
            if (grantResults.isEmpty()){
                println("grand result is empty ..")
            }
            if (!grantResults.all { it == PackageManager.PERMISSION_GRANTED }){
                println("grand results are not permission granded")
            }
            if (grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) {
                // Permissions granted, proceed
                startBleAdvertising("name") // <== Ensure `name` is in scope or store it as a class variable
            } else {
                Toast.makeText(this, "Permissions are required to start BLE advertising.", Toast.LENGTH_SHORT).show()
            }
        }
        else{
            println("its not request code permision")
        }
    }
